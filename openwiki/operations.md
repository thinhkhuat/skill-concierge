...
| `ENFORCER_JEV_GATE` | `1` | enforcer asks TypeSafe Jev (one Noul call, `TYPESAFE_API_KEY`) whether the turn needs a specialized skill, after the no-I/O lanes and before embed; p < `ENFORCER_JEV_SKIP_BELOW` (0.25) authorizes a 5th AUTHORIZED-SKIP leg (ledger band `jev_skip`); fail-open on no key / timeout (`ENFORCER_JEV_TIMEOUT` 1.2 s) / error; `=0` restores the 4-leg ladder | [0060](../docs/adr/0060-jev-needs-a-skill-gate.md) |
| `ENFORCER_DETERMINISTIC` | `1` | `config/deterministic-routes.json` substrings pin the named skill to the top of the offer (score 1.0, retrieved twin dropped), computed before embed so a timeout cannot lose it; `=0` disables (was default-inert before v0.47.0) | [0054](../docs/adr/0054-harness-message-lane-and-audit-fixes.md) |
