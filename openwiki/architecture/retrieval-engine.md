...
- Since `0.22.0` ([ADR-0031](../../docs/adr/0031-external-catalog-roots.md)), `discover_skills()`
  ...
  `catalog:<alias>` with `tier: external` on every point (since
  [ADR-0045](../../docs/adr/0045-catalog-tier-parity.md) the tier no longer excludes them —
  they compete in the merged per-turn offer at parity with installed skills; the tier now
  drives the `ENFORCER_EXTERNAL_OFFER` kill-switch and telemetry). Installed names always win
  collisions, and a catalog skill whose realpath matches an already-found skill (a promoted
  symlink) is suppressed.
...
- **LLM-utterance-derived** first (`SKILL_LLM_TRIGGERS` — off in code, **on in this deployment**):
  the offline flywheel-generated natural-utterance phrases (EN+VN) for this skill, read from the
  `llm_triggers` block of the canonical corpus `~/.claude/skill-concierge/triggers.json`
  (0.37.0; formerly the repo-local `eval/triggers.json`). Highest register-match, so they take the capped
  slots first. Generated by the flywheel
  ([ADR-0027](../../docs/adr/0027-flywheel-first-class-multi-provider.md)); layered by
  [ADR-0026](../../docs/adr/0026-llm-utterance-trigger-layer.md).

  > ✅ **Resolved in 0.37.0 (ADR-0044)** — this used to be the portability trap: the corpus
  > path lived machine-locally in `~/.claude/settings.json` `env` and the code defaults
  > resolved to unpopulated `eval/` trees, so an env-less reindex rebuilt **without**
  > utterance points, silently. Now the corpus is canonical at
  > `~/.claude/skill-concierge/triggers.json`, pinned in `.mcp.json` **and** the env-less
  > default in every generator and in the vendored engine — a bare reindex is safe by
  > construction. On a fresh machine, run one flywheel pass to generate the corpus
  > (desc/body-only triggers until then).
