...
| `SKILL_ROW_ORIGIN` (since `0.48.0`) | `1` (on) | ON (not overridden) — rows carry `origin`/`disabled_in` instead of `command` | code default |
| `SKILL_SYNCED_ROOTS` (since `0.48.0`) | `0` (off) | OFF (not overridden) — account-synced skills stay unindexed until every harness cache is ≥`0.48.0` | code default |

> **Do not read the code default as the deployed state.** `SKILL_LLM_TRIGGERS` is off in code but
...
Ranking and the trigger assembly live in [`server.py`](../../vendor/skill-search/skill_search/server.py);
the body-trigger **mining** and the purity lint (`_extract_body_triggers()`, `_is_impure_trigger()`)
live in [`skills_discovery.py`](../../vendor/skill-search/skill_search/skills_discovery.py).
...
- **Operator-curated first** (since `0.48.0`, [ADR-0058](../../docs/adr/0058-off-list-rule-exclusion-echo-row-provenance-synced-default-off.md)):
  `triggers-curated.json`, beside `SKILL_TRIGGERS` (no env var of its own), holds
  `{"<skill>": ["phrase", …]}` — phrases replayed from real routing misses, not generated, so they
  win the capped slots ahead of even the LLM layer. A malformed file fails open with one stderr
  line; absent file is byte-identical to before.
- **LLM-utterance-derived** next (`SKILL_LLM_TRIGGERS` — off in code, **on in this deployment**):
  the offline flywheel-generated natural-utterance phrases (EN+VN) for this skill, read from the
  `llm_triggers` block of `eval/triggers.json`. Highest register-match, so they take the capped
  slots first. Generated by the flywheel
  ([ADR-0027](../../docs/adr/0027-flywheel-first-class-multi-provider.md)); layered by
  [ADR-0026](../../docs/adr/0026-llm-utterance-trigger-layer.md).

  > ⚠ **`SKILL_TRIGGERS` is the portability trap.** The corpus path is **not** in `.mcp.json` — it
  > lives machine-locally in `~/.claude/settings.json` `env`. Unset, the code default resolves
  > relative to the *installed module* (inside the venv), where no `triggers.json` exists — so a
  > reindex with the flag on rebuilds **without** utterance points, silently and gracefully. On a
  > fresh machine, set `SKILL_TRIGGERS` or accept desc/body-only triggers.
...
33 MB request limit. A **per-project** manifest at `~/.cache/skill-search/index_meta-<md5(PROJECT_ROOT)[:8]>.json`
records the signature/model/dim so `health` can detect an embedder swap or a stale index (it was a
single shared `index_meta.json` before v0.19.0 — see [scoping](#multi-session-index-scoping--installed-plugin-filter-v0190-adr-0028)).
...
### Multi-session index scoping & installed-plugin filter (v0.19.0, ADR-0028)

Concurrent Claude Code sessions each run their own MCP server against **one shared Qdrant
collection**. Before v0.19.0, `SKILL_DIRS[1]` was `Path.cwd()/.claude/skills`, so every session saw
a different skill set and `build_index()` deleted any indexed point absent from *its* view — one
project's reindex silently wiped another project's points (last writer won, forever, on a 30-minute
...
[ADR-0033](../../docs/adr/0033-dual-harness-codex-parity.md), `catalog:<alias>` since
[ADR-0031](../../docs/adr/0031-external-catalog-roots.md), each other harness's own `-personal`/
`-plugin`/`-project:<root>` scopes, and `claude-synced` since
[ADR-0058](../../docs/adr/0058-off-list-rule-exclusion-echo-row-provenance-synced-default-off.md)
(Claude account-synced skills, default OFF)). `_prunable()` deletes **only** within `visible_scopes()`, and `search_skills` /
