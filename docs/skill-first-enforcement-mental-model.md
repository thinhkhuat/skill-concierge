...
## 12. Per-harness subagent doctrine scope (factual, 2026-08-26)

Whether a delegated subagent process receives the SKILL-FIRST doctrine, the per-turn enforcement mandate, and the candidate offer depends on which harness it runs under. The table below records the mechanism for each of the four supported harnesses — cited from source, with UNVERIFIED markers where the adapter surface does not provide definitive evidence.

| Harness | Doctrine+mandate+offer in subagents? | Mechanism | Source |
|---------|--------------------------------------|-----------|--------|
| Claude Code | **NO** | Subagents do **not** fire `UserPromptSubmit` — the enforcer hook (`hooks/hooks.json:36-51`, `UserPromptSubmit` → `enforcer.py`) never runs inside a subagent. The SessionStart `doctrine.py` (`hooks/scripts/doctrine.py:12-23`) suppresses injection when `agent_id` is present (ADR-0020, `_is_subagent()` at `doctrine.py:60-74`), and the per-turn trigger never fires because `UserPromptSubmit` has no subagent event. | `hooks/hooks.json:36-51`; `hooks/scripts/doctrine.py:12-23,60-74` |
| Oh My Pi (OMP) | **YES** — observed live 2026-08-26 | OMP fires `before_agent_start` for **every** agent start, including delegated subagents (`adapters/omp/skill-concierge.ext.ts:191-224`). The handler runs `runEnforcer()` (line 206) which embeds the prompt, queries Qdrant, and returns the mandate + candidate offer as a `customType: "skill-concierge"` message injected into the agent's context. The doctrine is injected per-agent via `before_agent_start` → the enforcer payload includes the standing order. Confirmed live from a subagent tour showing the doctrine banner at the subagent's context start. | `adapters/omp/skill-concierge.ext.ts:191-218`; `adapters/omp/skill-concierge.ext.ts:163-188` (SessionStart doctrine via `pi.on("session_start", ...)` — fires once per session, not per-agent) |
| Command Code | **UNVERIFIED** | The mod adapter (`adapters/commandcode/skill-concierge.mod.ts:78-107`) uses `transformInput` (`mod.ts:81`) to run the enforcer on every typed user prompt. Command Code's `transformInput` fires on user-entered text; whether subagent/internal prompts also pass through `transformInput` is not documented in the mod adapter code or in `docs/caveats.md` §21 (which records that `UserPromptSubmit` does not exist in Command Code settings hooks — `caveats.md:517-531`). The `skill_loaded`/`tool_completed` telemetry events (`mod.ts` further down, not shown) fire for tool activations, not for subagent lifecycle. Status: UNVERIFIED — the mod adapter does not address subagent behaviour, and Command Code's subagent architecture is not observable from the source surface. | `adapters/commandcode/skill-concierge.mod.ts:78-107`; `docs/caveats.md:521-531` |
| Codex | **UNVERIFIED** | Codex auto-discovers hooks from the plugin cache (`caveats.md §19`: ADR-0033). The hooks are Claude-format (`UserPromptSubmit`, `SessionStart`). Whether subagent sessions under Codex fire `UserPromptSubmit` is not documented in `.codex-plugin/` or `docs/caveats.md`. The `.codex/hooks.json` file exists only for the openwiki-parity guard (`PreToolUse` — `.codex/hooks.json:1-17`), not for skill-concierge enforcement (which ships in the plugin's own `hooks/hooks.json`). Since Claude Code subagents do not fire `UserPromptSubmit` and Codex inherits the same hook surface, the likely behaviour is also NO — but this is an inference, not source-confirmed. Status: UNVERIFIED. | `.codex/hooks.json:1-17`; `docs/caveats.md:466-478`, `docs/caveats.md:517-531` |

**Key architectural invariant across all four:** The doctrine injection gate — the distinction between a top-level session (gets the full doctrine) and a scoped worker (suppressed) — is decided by the SessionStart `doctrine.py` `_is_subagent()` check on `agent_id` (`doctrine.py:60-74`). This gate fires identically regardless of harness, because `doctrine.py` is a shared script invoked by each harness's own SessionStart mechanism. What varies is the **per-turn enforcement** — whether subagent prompts reach the enforcer at all. OMP's `before_agent_start` fires per-agent, so subagents get enforcement; the other harnesses' `UserPromptSubmit`/`transformInput` events scoped to user-entered text do not.

**Implication for the enforcement ledger:** Subagent rows under Claude Code, Command Code, and Codex carry neither a mandate nor an offer row — they never reach the enforcer. Under OMP, subagent rows DO carry enforcer output (offer, mandate, or fallback), so the ledger under OMP includes subagent-originated events that the other harnesses never produce. This is a known asymmetry, not a bug (the `_last_used_skill()` chain-hint reader explicitly excludes sub-stamped rows — `enforcer.py:571-574`).
