...
**Status:** P1 COMPLETE + **LIVE** (2026-06-26) — warm embed shim + enforcer hook built, verified (cosine parity 1.000000, fallback tested), **shipped as v0.2.0** then tuned in **v0.2.1** (threaded shim + 200ms embed timeout / ≲300ms budget after live dogfooding showed the 90ms cap caused ~60% mandate-only fallback under in-turn contention — see ADR-0008). Go-live executed: lexical `skill_first_nudge.py` deregistered from `~/.claude/settings.json` (backup `settings.json.bak-260626-pre-fusion-golive`), plugin updated 0.1.2→0.2.0, applied via `/reload-plugins`. Post-go-live verified live: exactly one enforcement hook fires, `band=offer` with semantic candidates when settled. Remaining: logman `RETENTION_DAYS=0` drop-in; measure the before/after uptake lift vs `plans/reports/baseline-260626-lexical-hook-snapshot.txt` as turns bank.
**Date:** 2026-06-26
...
**2026-08-29 — consult mode (ADR-0049): deliberation layer over the reflex layer (SHIPPED v0.42.0, `df9d888`; phase 2 routing in-session).**
- **Designed from a live practice run first**: the funnel (sieve → analyst deep-read → compose) executed manually on a real task before any code; evidence — 4 of the top-5 fits sieve-MISSED on vocabulary mismatch, a 0.86-scoring external dropped to LOW on body content — shaped the mechanism. Full record: `docs/ideas/consult-mode.md`.
- **v0.42.0**: `skills/consult/` funnel skill + `agents/analyst.md` template (strict-JSON contract, untrusted-bodies security); engine `consult_candidates` (≤5 sub-goal queries MAX-pooled, top_n≤40, externals first-class, path backfill via deterministic point id, `SKILL_CONSULT=0` kill-switch; `search_skills` byte-identical); capsule dossiers (`scripts/llm_capsules.py`, operator-home corpus, body+description fingerprint, opt-in `flywheel --generate --capsules`); `consult_log.py` verdict rows. Blind tester PASS; e2e probe both legs.
- **Phase 2 (same night, uncommitted at this entry)**: consult-intent routing in `hooks/scripts/enforcer.py` — `_CONSULT_RE` phrase class (8 fire / 6 silent selftest pins), leg after the refusal guard and BEFORE the embed I/O, subagent suppression via ADR-0020 `agent_id`, ledger kind `consult_route` carrying the prompt for false-route replay. Three live probes green (route / subagent fallthrough / kill-switch). Routed consults default `--fast`.
- **En-route defects fixed**: probe harness held stdin closed → server EOF'd before the in-flight tools/call reply (probe now holds stdin open, select-based); trigger-point payloads omit `path` → consult backfills via the same deterministic-id retrieve `get_skill` uses.

**2026-08-30 (early) — arc close: capsules complete, first real dish, phrase class v2 (SHIPPED 0.43.1, `fb808ca`).**
- **Capsule corpus 730/730**: bulk run finished 00:20 (710 clean / 20 fail-loud errors: malformed gateway JSON + shape-validation misses), tail retry recovered the remainder ~00:31. 3-sample spot-check: vocabulary distance from descriptions achieved, body-grounded facts, real avoid_when boundaries.
- **First real consult dish** (session 4ea5ee04, Mr Lam documents): the reflex offer's top picks had ZERO overlap with the deliberated chain (textutil+pandoc → tracked-diff audit → vn-editor review/polish → vn-author for the author's CAPS-flagged gaps → vn-gov-docx); one disclosed analyst override (local conversion over cloud parsing for a party-agency draft); verdict logged, user accepted. The ADR-0049 thesis demonstrated live.
- **Phrase class v2 from replayed evidence**: the dish's opening phrasing ("which set of skills that we should be using…") missed all three v1 anchors — windowed anchors + plural pronoun + `_CONSULT_NEG_RE` guards (past-conditional, reflexive past, "skills gap") per the W1 rule; pins 10 fire / 8 silent; the exact miss now routes, parked over-fires stay silent.
- Machine fix en route: Homebrew node@24 upgrade broke the unlazy Stop hook's versioned Cellar path → stable `/opt/homebrew/opt/node@24/bin/node` (rule banked: never versioned Cellar paths in configs).
