...
Claude Code, Codex, Command Code, and Oh My Pi (OMP): semantic retrieval (*which* skill) +
...
- `adapters/` — per-harness integration modules: `commandcode/` (mod adapter `skill-concierge.mod.ts` + install.sh + mcp.json, ADR-0038) and `omp/` (extension module `skill-concierge.ext.ts` loaded via root `package.json` `omp.extensions`, install.sh, mcp.json, ADR-0039). The OMP adapter never writes `~/.omp/agent/mcp.json` — OMP expands the plugin `.mcp.json`'s `${CLAUDE_PLUGIN_ROOT}` natively.
...
Each is a one-var revert to the prior behavior (`ENFORCER_AUTHORIZED_SKIP`, `ENFORCER_CHAIN_HINT`, `ENFORCER_CROSS_HARNESS`, `ENFORCER_MINED_CHAINS`, and `SKILL_BODY_TRIGGERS` default ON; `SKILL_LLM_TRIGGERS` default OFF):
