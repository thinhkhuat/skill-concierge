...
- `ENFORCER_EXTERNAL_OFFER` (`hooks/scripts/enforcer.py`) — catalog tier parity (ADR-0045, owner decision 2026-08-28 "no favor"): `_retrieve` issues ONE merged grouped query over BOTH tiers — no `must_not tier=external`, no separate annex query. Externals compete on score alone: same `ITEM_FLOOR` (0.18), same `TOP_K` slots, same %-share pool; they render INLINE marked `[external:<alias>]` (alias from the row's own `catalog:<alias>` payload scope) with one `get_skill` footer when any external row shows; catalog rows never take the ADR-0034 invocable-twin test (they are Skill-tool-invocable nowhere here — `get_skill` is their first-class lane). The getaway/actionability gates run tier-blind, so a strong external now carries a turn the installed shelf scores below floor on (the ADR-0032 annex could not). The sidecar admits `catalog:*` scopes, so CHAIN-HINT/ROUTE can name externals, tagged. `flywheel --generate` defaults to EVERY scope (installed first, then each configured catalog, each capped by `--limit`; `--installed-only` restores the pre-ADR-0045 default) and manifest records carry `scope`. Ledger `ext` keeps its `[name, score]` shape but lists external rows inside the PRIMARY offer — EPOCH v0.38.0 for offer-composition and external offer→take rates. `=0` (legacy `ENFORCER_EXTERNAL_ANNEX=0` honored) restores the ADR-0031 search-only tier exactly: the filter returns to the query, externals render nowhere, catalog scopes leave the visible sidecar. The pre-ADR-0034 byte-identical request shape additionally needs `ENFORCER_CROSS_HARNESS=0` (the scope payload now serves alias tagging). [ADR-0045](docs/adr/0045-catalog-tier-parity.md).
- `ENFORCER_ANNEX_DYNAMIC` (`hooks/scripts/enforcer.py`) — competitive-margin sizing for the ADR-0034 FOREIGN annex only since ADR-0045 (the external annex it used to size is gone): a foreign row earns its slot by scoring ≥ `max(pool floor, merged-pool top − ENFORCER_ANNEX_MARGIN)` (margin default 0.05 — measured: 0.10+ saturates on the compressed mpnet band, 0.05 discriminates), capped at `ENFORCER_FOREIGN_SLOTS` (2). Strong inventory → the annex shrinks toward 0; thin inventory → it widens to cap; deterministic hits silence it; no candidates → pool floor. The merged pool's `TOP_K` is untouched. `=0` reverts byte-identically to fixed sizing. [ADR-0036](docs/adr/0036-dynamic-annex-sizing.md).
