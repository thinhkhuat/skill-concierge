...
- `skills/{skill-search,setup,doctor,skill-usage-audit,keep-on,blocklist,flywheel,catalogs}/SKILL.md` — the plugin skills (router + maintenance + usage-audit + keep-on allowlist manager + blocklist disable-tier manager ADR-0046 + flywheel utterance coverage/generation ADR-0027 + external catalog-roots manager ADR-0031)
...
- `hooks/` — the in-generation governance layer: `enforcer.py` (per-turn SKILL-FIRST gate: embed→retrieve→floors/intent→ranked mandate, plus the AUTHORIZED-SKIP tier on its two silent legs — ADR-0015), `skill_guard.py` (PreToolUse(Skill) deny on the blocklist — the disable tier's deterministic layer, ADR-0046), `ledger.py` (invocation capture), `doctrine.py` (SessionStart standing-order injection), `auto_reindex.py` + `auto_overrides.py` + `auto_flywheel.py` (SessionStart self-heal: index + settings-override drift + utterance generation for new skills, ADR-0027 — fail-open, throttled, detached), `doctrine/skill-first.md` (the library doctrine text — burden of proof on SKIP, escalate to `find-skills`)
...
Each is a one-var revert to the prior behavior (`ENFORCER_AUTHORIZED_SKIP`, `ENFORCER_CHAIN_HINT`, `ENFORCER_CROSS_HARNESS`, `ENFORCER_MINED_CHAINS`, `ENFORCER_MULTI_INTENT`, `ENFORCER_CHAIN_PROJECTION`, `SKILL_BLOCKLIST`, and `SKILL_BODY_TRIGGERS` default ON; `SKILL_LLM_TRIGGERS` default OFF):
...
- `SKILL_LLM_TRIGGERS` (`vendor/skill-search/skill_search/server.py`) — default **OFF**; layers the offline flywheel-generated natural-utterance phrases (the `llm_triggers` block of the canonical corpus `~/.claude/skill-concierge/triggers.json`, EN+VN) FIRST in the MAX-pool trigger layer, ahead of description/body, capped COMBINED at `TRIGGERS_MAX` (live deploy uses `16` so utterances add slots rather than evict). Reads `SKILL_TRIGGERS` (path to the canonical corpus at `~/.claude/skill-concierge/triggers.json` —
