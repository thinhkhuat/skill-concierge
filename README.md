...
| `ENFORCER_JEV_GATE` | `1` (ON) | Jev needs-a-skill gate: one TypeSafe Jev Noul call (`TYPESAFE_API_KEY`) after the no-I/O lanes and before embed; p < `ENFORCER_JEV_SKIP_BELOW` (0.25) authorizes the skip (fifth `SKILL-CHECK:` leg, ledger band `jev_skip`). Fail-open on no key / timeout (`ENFORCER_JEV_TIMEOUT`, 1.2 s) / error; a named deterministic route never asks Jev; `ENFORCER_JEV_MODEL` pins `jev-1.13.0`. `=0` reverts. [ADR-0060](docs/adr/0060-jev-needs-a-skill-gate.md). |
