...
[![version](https://img.shields.io/badge/version-0.51.1-blue.svg)](CHANGELOG.md)
...
skill-concierge is a first-class citizen in all seven harnesses. Enforcement rides whatever
...
`0.51.0` — **published, ADR-0061 Jev skill router, replacing the ADR-0060 yes/no leg (which, replayed on real traffic, skipped 48 % of turns where the agent really used a skill). English prompts only: Jev ranks the whole invocable catalogue with conversation context, re-checks a shortlist with one `fits` question per candidate, and offers its top 5 — the skill the agent ends up using is in that offer 75 % of the time (177/237) vs 36 % for the embedding menu. Best fit < 0.30 authorizes the skip (band `jev_skip`, same locked signature). Warm `/jev` relay in the embed shim (~0.8-1.0 s per turn, overlapped with embed/Qdrant); any failure leaves the embedding path to decide. Calibrated on real outcomes by `scripts/calibrate_jev_gate.py`; no hand-labelled prompts. `ENFORCER_JEV_ROUTER=0` reverts. EPOCH v0.51.0.**

`0.50.0` — **published, ADR-0060 Jev needs-a-skill gate: one TypeSafe Jev Noul call, after every no-I/O lane and before embed, asks whether the turn needs a specialized skill at all. p < 0.25 takes a fifth `SKILL-CHECK:` leg ("Jev needs-a-skill gate", ledger band `jev_skip`) instead of the menu + search; anything else, and any timeout, HTTP error or missing key, falls through to the normal path (fail-open). A named deterministic route never asks Jev. Every ledger row after an attempted call carries `jev` (p or error, latency). Model pinned `jev-1.13.0`; typical call ~0.65 s, cap 1.2 s. `ENFORCER_JEV_GATE=0` reverts. EPOCH v0.50.0.**
...
`0.47.0` — **published, ADR-0054 harness-message lane + the v0.46.0 usage-audit fixes: prompts the harness generated (task notifications, monitor events, cross-session/teammate messages, idle reminders, OMP summarizer calls — 59 % of enforcer decisions in the audited epoch) are authorized to skip before any I/O; deterministic routes become config-driven, default ON and run before embed (a named skill leads even through a timeout); timeouts widened 0.35→0.5 s / 0.10→0.25 s (all epoch "outages" were censoring at the cap); keep-off activated in the durable home with a `doctor` check + `--fix`; `dsh-personal`/`cline-personal` foreign everywhere (no more bare twins); `analyze.py` fallback line counts outages only. EPOCH v0.47.0.**
`0.46.0` — **published, ADR-0053 cross-harness plugin-offer gates: the strict "only installed+enabled plugins are offered" rule now holds in EVERY harness — OMP joins the Claude session gate (namespaced plugin rows demand INVOCABLE_PLUGIN_IDS membership, the merged claude-layers + OMP-registry `enabled` view mirroring OMP's own loader; closes the live-proven ponytail@0.886 and enabled:false offers), DSH/Cline drop namespaced plugin rows outright (no skill-plugin registry exists — never invocable there), codex/commandcode/zcode keep their foreign/twin lanes (Codex own-scope TOML ungated = standing caveat). EPOCH v0.46.0.**
`0.45.0` — **published, ADR-0052 plugin-skills first-class fixes: layered enablement at index time (machine-wide UNION across user + every registered project's settings layers — user-false alone no longer erases a project-enabled plugin, live case agent-skills 25 skills), root-relative plugin scan with registry-derived ids (kills the `examples:workflow` phantom, old-version dedup, and temp_git clone hits structurally), and the per-session enablement gate `ENFORCER_PLUGIN_GATE` (Claude offers + chain-hint/ROUTE successors demand membership in the merged INVOCABLE_PLUGIN_IDS — live case ponytail offered where disabled). Also closes the 12 pre-existing engine-suite failures (ZCode/DSH/Cline seams unpinned in the hermetic conftest). Kill-switches: SKILL_PLUGIN_LAYERED_ENABLEMENT, SKILL_CLAUDE_PROJECTS_FILE, ENFORCER_PLUGIN_GATE. Offer-composition metrics change epoch at v0.45.0 — never pool across it.**
`0.44.1` — **published, ADR-0051 §4 caveat closed — Cline MCP tool naming live-verified: Cline CLI 3.0.60 is built on the Claude Agent SDK, so MCP tools surface FLATTENED as `<server>__<tool>` (`skill-search__search_skills`) — there is no `use_mcp_tool` in the model-facing surface; the doctrine's `cline` branch now renders the flattened form, the bridge captures the flattened lanes (args direct, no `{tool_name, arguments}` wrapper) and the live Skill tool name `skills`, and the phase-1 payload shape (`postToolUse:{toolName, parameters}`) was confirmed correct from the shipped binary's own hook-contract construction. ADR-0051 untouched (immutable); resolution recorded in CHANGELOG 0.44.1 + the ADR index row.**
`0.44.0` — **published, consult-intent phrase class v2 — widened from the FIRST live routed-task miss (replayed ledger evidence per the W1 rule): "which set of skills that we should be using…" slipped all three v1 anchors (intervening words, we-form, progressive), so v2 makes the which/what→skills→modal→i-we anchors windowed and pronoun-plural, with a negative guard (`_CONSULT_NEG_RE`) holding out the parked over-fires (past-conditional "should I have used", reflexive "did you use", org-talk "skills gap"). Pins 10 fire / 8 silent; the exact live-miss phrase routes; both parked over-fires stay silent; the tester's v1 under-fire "what skills can we use" now fires.**
`0.43.0` — **published, consult-intent routing (ADR-0049 phase 2): the enforcer detects deliberation-shaped turns ("which skills should I use for X", "plan a skill strategy", "best combo of skills", "consult which skills fit", "curate the skills") with a high-precision EN phrase class (8 fire / 6 silent selftest pins) and emits a CONSULT-ROUTE mandate — reply line 1 = USING: skill-concierge:consult — returning BEFORE the embed/retrieve pipeline (the funnel sieves itself; probes measured 43-85ms of reflex I/O saved per routed turn). Subagent sessions never route (ADR-0020 agent_id suppression); ledger kind `consult_route` carries the prompt verbatim for false-route replay (the epoch-watch v0.43.0 watch item — widen the phrase list only from replayed evidence). Routed consults default --fast. Kill-switch SKILL_CONSULT_ROUTE=0. Verified: enforcer selftest (new §12 pins), three live probes (route / subagent fallthrough / kill-switch revert).**
`0.42.0` — **published, consult — a deliberation layer over the reflex layer (ADR-0049; owner order: an opt-in "consult the best skill combo" planning step where the agent reasons deeply over the catalogue instead of the per-turn embedding offer; designed from a live practice run whose evidence — 4 of the top-5 fits sieve-MISSED on vocabulary mismatch, a 0.86-scoring external dropped to LOW on body content — shaped the mechanism): the `skill-concierge:consult` skill runs the funnel (distill sub-goals → wide `consult_candidates` sieve with one query per sub-goal → admit sieve misses → MANDATORY analyst-subagent deep body reads from `agents/analyst.md` with a strict-JSON contract → compose with session context → RUN/⚠/ALSO card → `consult_verdict` ledger row → route by flag, externals promote only on explicit accept). Engine: `consult_candidates` — the deliberate-lane sibling of `search_skills` (byte-identical untouched): MAX-pool over ≤5 sub-goal queries, `top_n` to 40, externals first-class with the read-inline marking, installed rows carry their body path (payload backfill via the deterministic point id — trigger points omit it), blocklist-filtered, `SKILL_CONSULT=0` kill-switch. Capsule dossiers: `llm_capsules.py` (purpose/capabilities/inputs/outputs/avoid_when, vocabulary deliberately off-description per the ADR-0026 v2 lesson) into the operator-home corpus `~/.claude/skill-concierge/capsules.json`, incremental over a body+description fingerprint, opt-in `flywheel.py --generate --capsules` (first bulk run operator-commissioned, ADR-0031 D10 doctrine), sieve attaches capsules LIVE-read, absent = description-only degrade. Phase 2 (enforcer consult-intent routing, `SKILL_CONSULT_ROUTE`) sequenced after live verification. EPOCH v0.42.0 for consult verdict→take, capsule coverage, sieve-admission rate. Verified: llm_capsules/consult_log selftests, e2e probe (server tools/list + live call, both legs), capsule attachment probe, driftcheck 0, doctor, blind tester.**
...
`0.35.1` — **published, the auto-flywheel goes fully autonomous over catalogs (ADR-0043 update): the `auto_flywheel` SessionStart hook now runs installed skills first, then one capped pass per configured external catalog alias (antigravity today) in the same detached run — steady-state cost stays bounded because the cache keys on skill content (only NEW/changed catalog skills reach the LLM) plus the MAX_PER_RUN cap, 6h throttle, and lock; new `AUTO_FLYWHEEL_WORKERS` (default 4) makes the detached pass ride the 1.9 s/skill parallel path (a capped 25-skill run: ~50 s vs ~7 min); and the `SKILL_AUTO_FLYWHEEL="false"` landmine in settings.json is defused to explicit `"1"` (inert under the `== "0"` gate but a silent-kill under any future strict-boolean parse).**
`0.35.0` — **published, ADR-0043 catalog-scoped flywheel generation + bounded parallel workers: the ADR-0031 D10 deferral lands as an explicit opt-in — `flywheel --generate --catalog <alias>` runs the utterance generator over ONE external catalog's `<alias>:*` skills (first commissioned run: `antigravity`, 1,928 skills; default coverage counts stay installed-only per the 0.22.1 contract; zero-indexed-catalog fails loud exit 5); `--workers <N>` (default 1, byte-identical) fans ONLY the LLM network phase out over a ThreadPoolExecutor while triggers.json/cache writes stay single-writer in the main thread — measured 16.0 s/skill sequential → 1.9 s/skill at 4 workers (21 skills/40 s, zero per-skill failures), 8.4 h → ~50 min for the commissioned catalog; and `chat()` now routes the gateway's HTTP-200-wrapped upstream 503s (`[CommandCode error: {… "statusCode":503,"isRetryable":true}]` with `finish_reason: "stop"` — invisible to the 5xx ladder, and the RuntimeError that killed the first catalog pass) through the same 3-attempt ladder, raising URLError (an OSError every per-skill handler catches) on exhaustion so a sustained outage fails the skill, never the pass. Model head-to-head recorded: `cmc/xiaomi/mimo-v2.5` rejected at 6-8× the latency of `cmc/MiniMaxAI/MiniMax-M3`. Scenario layer excluded for catalog runs (`--triggers-only`, no external consumer).**
